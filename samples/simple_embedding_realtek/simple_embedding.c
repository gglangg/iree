...
iree_status_t Run() {
  ...
  float kFloatArg0[arraySize];
  float kFloatArg1[arraySize];
  float relAnswer[arraySize];
  for(int i=0;i<arraySize;i++){
    kFloatArg0[i] = (float)rand()/(float)(RAND_MAX/3.14);
    kFloatArg1[i] = (float)rand()/(float)(RAND_MAX/3.14);
    relAnswer[i] = kFloatArg0[i]+kFloatArg1[i];
  }
  ...
  iree_hal_dim_t shape[1] = {IREE_ARRAYSIZE(kFloatArg0)};
  ...
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_allocate_buffer_copy(
      ...
      iree_make_const_byte_span(kFloatArg0, sizeof(kFloatArg0)), &arg0_buffer_view));
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_allocate_buffer_copy(
      device, iree_hal_device_allocator(device), IREE_ARRAYSIZE(shape), shape,
      IREE_HAL_ELEMENT_TYPE_FLOAT_32, IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
      (iree_hal_buffer_params_t){
      ...
      iree_make_const_byte_span(kFloatArg1, sizeof(kFloatArg1)), &arg1_buffer_view));
  ...
  // Read back the results and ensure got the right values.
  // /*
  float results[arraySize];
  ...
  // clhuang: comment this to disable answer check.
  /* 
  for (iree_host_size_t i = 0; i < IREE_ARRAYSIZE(results); ++i) {
    if (results[i] != relAnswer[i]) {
      printf("results[%d] = %f\n, rel_answer[%d] = %f\n", (int)i, results[i], (int)i, relAnswer[i]);
